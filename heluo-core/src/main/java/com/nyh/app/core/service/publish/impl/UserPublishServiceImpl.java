...
public class UserPublishServiceImpl implements UserPublishService {
	...
	public PublishVo saveimage(MultipartFile image){
		...
		File pub_dir = new File("/Users/niu_xd/Downloads/apache-tomcat-8.0.45/webapps/ROOT/images/"+userId+"/"+dircount);
		...
		if(mkdirs){
			 try {
				 ...
				 String upload_image = "http://172.16.41.215:8888/images/"+userId+"/"
	...
	public PublishVo savecontent(PublishVo pub) {
		...
		if (pub == null) {
			throw new ServiceException("请输入发布内容");
		}
		String userId = WebContext.getUserId();
		...
		String sub_imagUrl = "";
		if (pub.getImageUrl() != null) {
			sub_imagUrl = pub.getImageUrl().substring(1);
		}
